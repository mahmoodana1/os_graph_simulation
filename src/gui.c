...
//     *c1 = (Vector2){from.x + dir.x*0.33f + perp.x*curve, from.y + dir.y*0.33f + perp.y*curve};
//     *c2 = (Vector2){to.x   - dir.x*0.33f + perp.x*curve, to.y   - dir.y*0.33f + perp.y*curve};
// }
//
// static void DrawRoadSign(Vector2 p0, Vector2 c1, Vector2 c2, Vector2 p3, float t, int w, bool on_path)
// {
...
//     Vector2 postBase = {roadPt.x + rperp.x*(ROAD_THICK*0.5f+3.0f), roadPt.y + rperp.y*(ROAD_THICK*0.5f+3.0f)};
...
// {
//     if (car->path_len <= 0) { car->state = CAR_ARRIVED; return; }
//
//     car->timer += dt;
...
//             while (e) {
//                 if (e->id == car->path[car->seg+1]) { car->total_hops = e->weight; break; }
//                 e = e->next;
//             }
...
//     if (!ctx->cars) { fprintf(stderr, "Error: malloc failed for cars\n"); exit(1); }
//
...
//     }
//
//     Color traveler_colors[] = {
//         {230,  50,  50, 255},   /* red    */
//         { 50, 120, 220, 255},   /* blue   */
//         { 40, 180, 100, 255},   /* green  */
//         {220, 160,  40, 255},   /* yellow */
//         {160,  80, 180, 255},   /* purple */
//         {240, 120, 180, 255}    /* pink   */
//     };
//
...
//
//     /* Update all cars that haven't arrived yet */
//     if (ctx->playing) {
...
//
//     /* 2. All roads — dark base with arrows and weight signs */
//     for (int i = 0; i < g->num_nodes; i++) {
//         Node* e = g->adj[i];
//         while (e) {
...
//     }
//
//     /* 3. Path overlays — each traveler's route in their own color */
//     for (int c = 0; c < ctx->numCars; c++) {
//         Car* car = &ctx->cars[c];
//         for (int k = 0; k + 1 < car->path_len; k++) {
//             int u = car->path[k], v = car->path[k+1];
...
//             DrawSplineBezierCubic(ppts, 4, ROAD_THICK+6.0f, car->color);
...
//     for (int i = 0; i < ctx->numCars; i++) {
//         Car* car = &ctx->cars[i];
//         if ((car->state == CAR_MOVING || car->state == CAR_NODE_WAIT) && car->seg+1 < car->path_len) {
//             Vector2 from = ctx->positions[car->path[car->seg]];
//             Vector2 to   = ctx->positions[car->path[car->seg+1]];
//             Vector2 cc1, cc2;
//             GetEdgeBezier(from, to, &cc1, &cc2);
//             float denom = (float)(car->total_hops > 0 ? car->total_hops : 1);
//             float t = (car->state == CAR_MOVING)
//                 ? fmaxf(0.0f, fminf((car->hop + fminf(car->timer/HOP_DURATION_SEC, 1.0f)) / denom, 1.0f))
//                 : 0.0f;
//             Vector2 tang = GetBezierTangent(from, cc1, cc2, to, t);
//             float angle  = (Vector2Length(tang) > 0.1f) ? atan2f(tang.y, tang.x)*(180.0f/PI) : 0.0f;
//             DrawRectanglePro((Rectangle){car->x+4,car->y+4,22,12},(Vector2){11,6},angle,(Color){0,0,0,70});
//             DrawRectanglePro((Rectangle){car->x,  car->y,  22,12},(Vector2){11,6},angle,car->color);
//         }
//     }
//
//     /* 6. All-arrived banner */
//     bool all_arrived = (ctx->numCars > 0);
//     for (int i = 0; i < ctx->numCars; i++)
//         if (ctx->cars[i].state != CAR_ARRIVED) { all_arrived = false; break; }
//
//     if (all_arrived) {
...
//         DrawText("ALL TRAVELERS ARRIVED",(int)(banner.x+60),(int)(banner.y+14),20,WHITE);
//         DrawText("All shortest paths completed.",(int)(banner.x+60),(int)(banner.y+40),13,(Color){190,200,210,200});
//         float rbW=150.0f, rbH=36.0f;
//         Rectangle restartBtn = {banner.x+(bW-rbW)*0.5f, banner.y+bH-rbH-14.0f, rbW, rbH};
...
//         DrawText(rbText,(int)(restartBtn.x+(rbW-MeasureText(rbText,16))*0.5f),
...
bool RenderFrame(RenderCtx* ctx, Graph* g, float dt)
{
    ...
    /* ملاحظة: تم حذف حلقة الـ Path overlays من هنا لتبقى الطرق بيضاء */
