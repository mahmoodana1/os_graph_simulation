...
static void DrawHeart(float cx, float cy)
{
    ...
    DrawCircle(cx + s / 2, cy - s / 2, s / 2, MM_HEART);
    DrawTriangle((Vector2){cx - s - 1, cy - s / 2 + 1}, (Vector2){cx, cy + s + 2},
                 (Vector2){cx + s + 1, cy - s / 2 + 1}, MM_HEART);
}
...
static void DrawArrow(Vector2 start, Vector2 end, Color c)
{
    ...
    DrawTriangle(tip, (Vector2){base.x + unit.y * aHalf, base.y - unit.x * aHalf},
                 (Vector2){base.x - unit.y * aHalf, base.y + unit.x * aHalf}, c);
}
...
static void UpdateCar(Car* car, RenderCtx* ctx, Graph* g, float dt)
{
    ...
    else if (car->state == CAR_MOVING)
    {
        ...
        car->x = s.x + prog * (d.x - s.x);
        car->y = s.y + prog * (d.y - s.y);
...
bool RenderFrame(RenderCtx* ctx, Graph* g, float dt) {
    ...
    for (int i = 0; i < g->num_nodes; i++) {
        ...
        while (e) {
            ...
            if (!on) {
                DrawRoad(ctx->positions[i], ctx->positions[e->id], ROAD_THICK, MM_ROAD);
    ...
    for (int i = 0; i + 1 < ctx->dijk_len; i++) {
        int u = ctx->dijk_path[i];
        int v = ctx->dijk_path[i + 1];

        DrawRoad(ctx->positions[u], ctx->positions[v], ROAD_THICK + 6, MM_ROAD_PATH);

        int w = 1;
        Node* tmp = g->adj[u];
        while (tmp) {
            if (tmp->id == v) { w = tmp->weight; break; }
            tmp = tmp->next;
        }

        DrawWeightBadge(ctx->positions[u], ctx->positions[v], w, true);
    }
    ...
    if (ctx->playing && (ctx->car.state == CAR_MOVING || ctx->car.state == CAR_NODE_WAIT)) {
        DrawHeart(ctx->car.x, ctx->car.y);
    }

    int bx = SCREEN_W - 116, by = SCREEN_H - 50;
    Rectangle btn = {(float)bx, (float)by, 100, 36};
